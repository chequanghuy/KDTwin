...
import sys
from TwinPlus import config as cfg 
...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        return out_encoder,output0,output1,output2,inp1,inp2
...
class TwinLiteNetPlus(nn.Module):
    ...
    def __init__(self, args=None, student = False):
        ...
        self.caam = CAAM(feat_in=model_cfg['chanels'][2], num_classes=model_cfg['chanels'][2],bin_size =(2,4), norm_layer=nn.BatchNorm2d)
        self.conv_caam = ConvBatchnormRelu(model_cfg['chanels'][2],model_cfg['chanels'][1])

        self.up_1_da = UpConvBlock(model_cfg['chanels'][1],model_cfg['chanels'][0]) # out: Hx4, Wx4
        self.up_2_da = UpConvBlock(model_cfg['chanels'][0],8) #out: Hx2, Wx2
        self.out_da = UpConvBlock(8,2,last=True)  

        self.up_1_ll = UpConvBlock(model_cfg['chanels'][1],model_cfg['chanels'][0]) # out: Hx4, Wx4
        self.up_2_ll = UpConvBlock(model_cfg['chanels'][0],8) #out: Hx2, Wx2
    ...
    def forward(self, input):
        ...
        out_da_0=self.up_1_da(out_caam,inp2)
        out_da_1=self.up_2_da(out_da_0,inp1)
        out_da_logit=self.out_da(out_da_1)

        out_ll_0=self.up_1_ll(out_caam,inp2)
        out_ll_1=self.up_2_ll(out_ll_0,inp1)
        out_ll_logit=self.out_ll(out_ll_1)
